...
int main(int argc, char* argv[]){
    ...
    DVector DmainInitialDataH;
    inOut.initialValueFile >> DmainInitialDataH;
    IVector ImainInitialData(DmainInitialData);
    IVector ImainInitialDataH(DmainInitialDataH);
    int mainC0Size;
    int fullC0Size;
    ImainInitialData= ImainInitialData;
    double eps;
    ...
    Series startingSet(ImainInitialData,C,s,SeriesType::sin);
    ...
    Series u = startingSet;
    Series h;
    //u.print();
    cout << "Initial data:"<<endl;
    u.resize(NumberOfPrintedModes).print();
    double C_h;
    inOut.initialValueFile >> C_h;
    double s_h;
    inOut.initialValueFile >> s_h;
    h = Series(IVector(ImainInitialDataH),interval(C_h)*interval(-1,1),interval(s_h),SeriesType::sin);
    cout << "Initial data:"<<endl;
    h.resize(NumberOfPrintedModes).print();
    auto result = C1Computation(u,h,mainC1Size,fullC1Size,paramsDVector);
    result.resize(NumberOfPrintedModes).print();
